...
class ScheduleView(ft.Column):
    def __init__(self, page: ft.Page, schedule, change_screen_func):
        ...
        self.bottom_nav = ft.Container(content=self.bottom_nav_row, visible=self.is_native_mobile, bgcolor="surface", border=ft.border.only(top=ft.border.BorderSide(1, "outlineVariant")), padding=ft.padding.only(top=10, bottom=20))
        ...
        top_controls = [header]
        if not self.is_native_mobile: top_controls.append(self.tabs_row)
            
        self.controls = [ft.Column(top_controls), ft.Stack([self.content_area, ft.Container(content=add_btn, bottom=20, left=20)], expand=True), self.bottom_nav]

    def get_tabs(self):
        return [t("schedule.tab_weekly"), t("schedule.tab_lectures"), t("schedule.tab_stats")] if self.is_narrow_screen else [t("schedule.tab_weekly"), t("schedule.tab_lectures")]
    ...
    def handle_resize(self, e):
        ...
        if new_is_narrow != self.is_narrow_screen:
            ...
            if not self.is_native_mobile: self.build_tabs()
            self.update_content()
            self.update()
    ...
    def open_add_menu(self, e):
        ...
        if self.selected_tab == t("schedule.tab_lectures"):
            options_content = ft.Column([
                ft.Text(t("schedule.add_task", default="משימה חדשה"), size=18, weight="bold", color="onSurface"), ft.Divider(color="outlineVariant"),
            ...
            options_content = ft.Column([
                ft.Text(t("schedule_menu.add_options"), size=18, weight="bold", color="onSurface"), ft.Divider(color="outlineVariant"),
    ...
    def open_oneoff_event_dialog(self):
        ...
        def save_oneoff(e):
            if not course_dropdown.value or not title_input.value: return
            ...
            if course:
                ...
                lec = LectureSession(
                    session_id=session_id, title=f"{course.title} - {title_input.value}",
                    lecturer=course.lecturer, date_obj=datetime.now().date(),
                    duration_mins=dur, is_one_off=True, meeting_type=type_dropdown.value, status=LectureStatus.NEEDS_WATCHING
                )
                lec.course_color = course.color
                course.lectures.append(lec)
                self.schedule.save_to_file(); self.refresh_ui(); dlg.open = False; self.app_page.update()
                
        def close_dialog(e): dlg.open = False; self.app_page.update()
        ...
        dlg = ft.AlertDialog(
            ...
            actions=[ft.TextButton(t("common.cancel", default="ביטול"), on_click=close_dialog), ft.ElevatedButton(t("common.save", default="שמור"), on_click=save_oneoff, bgcolor="primary", color="onPrimary")]
        )
        self.app_page.overlay.append(dlg); dlg.open = True; self.app_page.update()

    def build_tabs(self):
        self.tabs_row.controls.clear()
        for tab in self.tabs:
            is_selected = (tab == self.selected_tab)
            btn = ft.TextButton(content=ft.Text(tab, color="onSecondaryContainer" if is_selected else "onSurfaceVariant", weight="bold" if is_selected else "normal"), style=ft.ButtonStyle(bgcolor="secondaryContainer" if is_selected else "transparent", shape=ft.RoundedRectangleBorder(radius=20)), on_click=self.create_tab_click_handler(tab))
        ...
        for icon_name, tab_name in nav_items:
            ...
            nav_btn = ft.Container(content=ft.Column([ft.Icon(icon_name, size=24, color=color), ft.Text(tab_name, size=11, color=color, weight="bold" if is_selected else "normal")], spacing=4, alignment=ft.MainAxisAlignment.CENTER, horizontal_alignment=ft.CrossAxisAlignment.CENTER), expand=True, ink=True, padding=ft.padding.symmetric(vertical=8), border_radius=10, on_click=self.create_tab_click_handler(tab_name))
            self.bottom_nav_row.controls.append(nav_btn)

    def create_tab_click_handler(self, tab):
        def on_click(e): self.selected_tab = tab; self.build_tabs(); self.update_content(); self.update()
        return on_click
    
    def refresh_ui(self):
        self.schedule.save_to_file(); self.weekly_grid_component.update_grid(); self.lectures_list_component.update_list(); self.update_content(); self.update()

    def update_content(self):
        if self.selected_tab == t("schedule.tab_weekly"): main_view = self.weekly_grid_component
        elif self.selected_tab == t("schedule.tab_stats"): main_view = StatisticsPanel(self.schedule)
        elif self.selected_tab == t("schedule.tab_lectures"): main_view = self.lectures_list_component
