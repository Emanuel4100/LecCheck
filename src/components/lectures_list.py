    ...
    def update_list(self):
        ...
        for f_name in filters:
            ...
            btn = ft.Container(
                ...
                on_click=lambda e, fn=f_name: self.change_filter(fn), ink=True
            )
        ...
        if self.current_sort_method == "duration": lectures.sort(key=lambda x: x.duration_mins or 0, reverse=True)
        elif self.current_sort_method == "type": lectures.sort(key=lambda x: str(x.meeting_type))
        else: lectures.sort(key=lambda x: (x.date_obj, x.start_time if x.start_time else "00:00"))
        ...
        if not lectures:
            ...
            for lec in lectures: list_view.controls.append(LectureCard(lec, self.refresh_callback, is_mobile=False, show_date=True))

        self.controls = [ft.Container(content=top_bar, padding=ft.padding.only(top=10, bottom=5, left=10, right=10)), list_view]