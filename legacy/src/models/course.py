...
class Course:
    ...
    def _generate_lectures_for_rule(self, semester_start, semester_end, rule, preserved_data=None):
        ...
        while current_date <= semester_end:
            if current_date.weekday() == target_weekday:
                ...
                if preserved_data and (date_str, rule["start_time"]) in preserved_data:
                    ...
                    ext_link = saved.get("link", "")
                    custom_duration = saved.get("duration_mins")
                ...
                if custom_duration is not None:
                    new_lec.duration_mins = custom_duration
                else:
                    try:
    ...
    def recalculate_all_lectures(self, new_start, new_end, enable_numbers=True):
        ...
        for rule in self.meetings:
            self._generate_lectures_for_rule(new_start, new_end, rule, preserved_data)
